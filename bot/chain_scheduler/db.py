...
from sqlalchemy import create_engine, desc, func, asc, or_, not_
...
def apply_query_filters(query, model, filters):
    for f in filters:
        match f[1]:
            ...
            case "in":
                query = query.filter(getattr(model, f[0]).in_(f[2]))
            case "not_in":
                query = query.filter(not_(getattr(model, f[0]).in_(f[2])))
            case "or":
...
async def edit_or_write(db: Session, table: str, data: dict, filters: list):
    if not await edit_entries(db, table, data, filters, 1):
        print("WRITING?")
        await write_or_dont(db, table, data, filters)
    else:
        print("EDITED?")
    return True
...
def to_dict(row):
    return copy.deepcopy(row.__dict__)
