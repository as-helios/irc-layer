...
from sqlalchemy import create_engine, func, asc, desc, or_
...
IRCLogNickPrune = Base.classes.irc_log_nick_prune
IRCBannedHostmasks = Base.classes.irc_banned_hostmasks
IRCBannedHostnames = Base.classes.irc_banned_hostnames
IRCBannedNicks = Base.classes.irc_banned_nicks
IRCBannedPhrases = Base.classes.irc_banned_phrases
...
def edit_entries_sync(db: Session, table: str, data: dict, filters: list = [], amount: int = 1):
    model = get_model_from_table(table)
    query = db.query(model)
    query = apply_query_filters(query, model, filters)
    if amount not in (-1, 0, 1):
...
def get_entries_sync(db: Session, table: str, filters: list = [], amount: int = 0, sort_by: str = None, sort_order: str = "desc", columns: list = []):
    model = get_model_from_table(table)
    if not columns:
        query = db.query(model)
    else:
